    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.n_epochs):
            ...
            for start in range(0, n_samples, self.batch_size):
                ...
                self.w = (1 - eta_t * self.lambda_param) * self.w - eta_t * grad_hinge
...
class Trainer:
    ...
    def train(self):
        ...
        for j, col in enumerate(LABEL_COLS):
            ...
            print(
                f"  pred 1-rate (raw train) = {pred_pos_rate:.4f}, "
                f"train acc = {acc_train:.4f}"
            )
            print(
                f"  score range on train    = [{scores_train.min():.4f}, "
...
class Inference:
    ...
    def predict_single(self, text: str):
        ...
        scores = np.zeros((embedding.shape[0], len(LABEL_COLS)), dtype=np.float32)
        for j, label in enumerate(LABEL_COLS):
            scores[:, j] = self.svm[label].decision_function(embedding)
        probs = expit(scores)
...
if __name__ == "__main__":
    not_retrain = True  # False -> always retrain the model
